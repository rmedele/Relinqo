# Relinqo Outreach Email Copy

Use these manually, one email at a time. Personalize the first sentence from the prospect tracker before sending.

## Subject Options

- Quick idea for missed quote requests
- Free 30-day lead response pilot
- Can I test this on one real lead?
- Helping local service businesses reply faster

## First 5 Email

Hi {{first_name_or_team}},

I found {{company}} while looking for local Edmonton service businesses that handle urgent customer requests. I am building Relinqo, a small AI front-desk system for trades companies.

It connects to Gmail and missed-call SMS, spots quote requests and urgent jobs, drafts a reply, and texts the owner when a hot lead comes in. The point is simple: reply in minutes, not hours.

Would you be open to letting me run it free for 30 days on one real lead source? If it helps you win even one job, keep it for $99/mo. If not, I will shut it off and you owe nothing.

Here is the short page: https://www.relinqo.com/book-demo

No worries if this is not relevant, reply "no" and I will not follow up.

Thanks,
Reese
Relinqo
https://www.relinqo.com

## Short Reply-Led Version

Hi {{first_name_or_team}},

Quick question: do quote requests or missed calls ever sit too long before someone replies?

I am building Relinqo for local trades businesses. It catches Gmail and missed-call leads, drafts the first response, and texts the owner when the job looks urgent.

I am offering one free 30-day pilot in Edmonton. Can I show you what it would do with one real lead from {{company}}?

No worries if this is not relevant, reply "no" and I will not follow up.

Thanks,
Reese
Relinqo
https://www.relinqo.com/book-demo

## Follow-Up After 3-4 Business Days

Hi {{first_name_or_team}},

Just bumping this once. I am looking for one local service business to test Relinqo free for 30 days.

The test is lightweight: connect one lead source, keep human approval on, and see whether faster replies help book more work.

Worth a quick look, or should I leave you alone?

Thanks,
Reese

No worries if this is not relevant, reply "no" and I will not follow up.

## Starter Reply Templates For Relinqo

Quote request:

```text
Hi {{name}}, thanks for reaching out to {{business}}. We can help with this. Could you send the service address, a few photos if safe, and the best number to reach you? Once we have that, we can give you the next available time and a rough quote range.
```

Scheduling:

```text
Hi {{name}}, we can get this scheduled. Please use this link to pick a time that works for you: {{booking_link}}. If this is urgent, reply with "urgent" and the best callback number.
```

Thank-you / won job:

```text
Hi {{name}}, thanks again for choosing {{business}}. We appreciate the chance to help. If anything comes up after the visit, reply here and we will take care of it.
```
