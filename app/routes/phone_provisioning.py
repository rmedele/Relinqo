...
"Create my business line" in the reqlinqo UI, we buy him a number
...
def _forwarding_code(rescue_number: str, carrier: str | None) -> dict:
    ...
    return {
        ...
        "steps": [
            ...
            "Come back to reqlinqo and run the test.",
        ],
    }
...
def rescue_setup(
    ...
    reqlinqo finds, buys, configures, and saves routing for a number.
    ...
    try:
        ...
        result = provision_number(
            ...
            voice_url=voice_url,
            status_callback_url=status_url,
            friendly_name=f"reqlinqo rescue line {chosen}",
        )
    ...
    row = PhoneNumber(
        ...
        twilio_sid=result["sid"],
        phone_number=result["phone_number"],
        friendly_name=result.get("friendly_name") or f"reqlinqo rescue line {chosen}",
        is_active=True,
    )
    db.add(row)
    rule = _upsert_routing_rule(
...
def rescue_forwarding_setup(
    ...
    calls to a reqlinqo rescue number, then test that forwarding works.
    """
    area_code = (payload.area_code or "").strip()
    if not re.match(r"^\d{3}$", area_code):
        raise HTTPException(status_code=400, detail="Enter a 3-digit area code.")
    ...
    if row is None:
        ...
        try:
            ...
            result = provision_number(
                ...
                voice_url=voice_url,
                status_callback_url=status_url,
                friendly_name=f"reqlinqo missed-call rescue {chosen}",
            )
        except HTTPException:
            raise
        except TwilioError as e:
        ...
        row = PhoneNumber(
            ...
            twilio_sid=result["sid"],
            phone_number=result["phone_number"],
            friendly_name=result.get("friendly_name") or f"reqlinqo missed-call rescue {chosen}",
            is_active=True,
        )
        db.add(row)
...
def adopt(
    ...
    point at reqlinqo, and saves it to the org. Optionally writes the
    ...
    if not remote:
        raise HTTPException(
            status_code=404,
            detail=f"Number {phone} not found on this Twilio account. Buy it in the Twilio Console first, or use /provision to buy via reqlinqo.",
        )
    ...
    try:
        update_number_webhooks(
            ...
            status_callback_url=status_url,
            sms_url=sms_url,
            friendly_name=f"reqlinqo {phone}",
        )
    except TwilioError as e:
